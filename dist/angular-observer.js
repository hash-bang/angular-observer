...
angular.module('angular-observer', []).provider('$observeProvider', function () {
	var _this = this;
 ...
 */
	this.deregister = function (observer) {
		var oIndex = this.observers.findIndex(function (o) {
			return o == observer;
		});
 ...
 */
	...
	var $observe = function $observe(scope, paths, callback, params) {
  ...
  */
		...
		if (observe.root === true && observe.paths.length == 1) {
			// Auto compute the root if root==true and we only have one path anyway
  ...
  */
		observe.traverse = function (cb, path, item, depth) {
			if (!path && _.isUndefined(item)) {
				// If we we're passed undefined assume the user wanted to traverse all specified paths
				observe.paths.forEach(function (path) {
					return observe.traverse(cb, path.split('.'), observe.get(path), 1);
				});
			} else if (_.isArray(item)) {
				if (_.isNumber(observe.deep) && depth > observe.deep) return; // Refuse to go any deeper
				item.forEach(function (v, k) {
					return observe.traverse(cb, (path || []).concat([k]), v, depth ? depth + 1 : 1);
				});
			} else if (_.isObject(item)) {
				if (_.isNumber(observe.deep) && depth > observe.deep) return; // Refuse to go any deeper
				_.keys(item).forEach(function (k) {
					return observe.traverse(cb, (path || []).concat([k]), item[k], depth ? depth + 1 : 1);
				});
			} else {
		...
		observe.inject = function () {
			...
			observe.traverse(function (v, k, path) {
				...
				if (_.isObject(v)) {
					...
					// For everything else - stash the original value in this.parent.$originalValues
  ...
  */
		observe.isModified = function (path) {
			if (path) {
				...
				if (_.isObject(v)) {
					// If its an object (or an array) examine the $clean propertly
					return !v.$clean;
				} else {
					// If its everything else look at the original value we have on file
  ...
  */
		observe.emit = function (hook) {
			...
			observe.hooks[hook].forEach(function (hook) {
			...
			if (removeOnce) observe.hooks[hook] = observe.hooks[hook].filter(function (hook) {
				return !hook.once;
			});
  ...
  */
		observe.off = function (hook, cb) {
			...
			observe.hooks[hook] = observe.filter(function (h) {
				if (_.isFunction(cb)) {
					return h.cb == cb;
				} else {
					return h.id == cb;
  ...
  */
		observe.destroy = function () {
			$observeProvider.deregister(observe);
		...
		observe.inject();
		$timeout(function (_) {
			return observe.emit('init');
		}); // Fire init on the next cycle so everything has the chance to register
		return observe;
	};
